	...
	while (rdma_get_cm_event(ec, &event) == 0)
	{
		...
		switch (event_copy.event)
		{
			case RDMA_CM_EVENT_ADDR_RESOLVED:
				{
					...
					if (s_on_pre_conn_cb)s_on_pre_conn_cb(event_copy.id);
			...
			case RDMA_CM_EVENT_ESTABLISHED:
				{
					if (s_on_connect_cb) s_on_connect_cb(event_copy.id);
				}
				break;
			case RDMA_CM_EVENT_DISCONNECTED:
				{
					printf("Received a Terminated request....\n");
					rdma_destroy_qp(event_copy.id);
					if (s_on_disconnect_cb)s_on_disconnect_cb(event_copy.id);
...
void * poll_cq(void *ctx)
{
	...
	while (1)
	{
		...
		while (ibv_poll_cq(cq, 1, &wc))
		{
			...
			{
				printf("IBV_WC_RETRY_EXC_ERR for %d, will sleep for 10 seconds\n",
				       ++retry_loops);
				sleep(10);
				if (ibv_req_notify_cq(cq, 0) != 0)
				{
					printf("error in ibv_req_notify_cq\n");
				}
				if (retry_loops > 100)
				{
					printf("IBV_WC_RETRY_EXC_ERR for 100 times\nwc = %s\n",
