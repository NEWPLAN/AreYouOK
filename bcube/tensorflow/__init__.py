...
from bcube.tensorflow.bcube_ops import size
from bcube.tensorflow.bcube_ops import rank
from bcube.tensorflow.bcube_ops import local_rank
from bcube.tensorflow.bcube_ops import allgather
from bcube.tensorflow.bcube_ops import broadcast
from bcube.tensorflow.bcube_ops import _allreduce
from bcube.tensorflow.bcube_ops import init
...
def allreduce(tensor, average=True, device_dense='', device_sparse=''):
    ...
    if isinstance(tensor, tf.IndexedSlices):
        with tf.device(device_sparse):
            ...
            bcube_size = tf.cast(size(), tensor.values.dtype)
            values = allgather(tensor.values)
            indices = allgather(tensor.indices)

            # To make this operation into an average, divide all gathered values by
            # the Bcube size.
            new_values = tf.div(values, bcube_size) if average else values
        return tf.IndexedSlices(new_values, indices,
                                dense_shape=tensor.dense_shape)
    else:
        with tf.device(device_dense):
            bcube_size = tf.cast(size(), tensor.dtype)
            summed_tensor = _allreduce(tensor)
            new_tensor = (tf.div(summed_tensor, bcube_size)
